...
<body>
        ...
        --%>
         ...
         <tr>
           <td colspan="5" class="text-center">
             ...
             <input type=button value="디음" class="btn-sm btn-info">
           </td>
         </tr>
         </tfoot>
      </table>
  ...
  <script>
    let dataApp=Vue.createApp({
    	...
    	// 멤버변수의 값이 변경이 된 경우 => Component 제작
    	// Callback => Vue에 의해 '자동 호출;되는 함수
    	updated(){
    		
    	},
    	// 사용자 정의 메소드 => 이벤트 => 멤버 메소드
    	methods:{
    		// 서버에서 데이터를 읽어온다 => 이전/다음/시작 => 반복제거의 방법:메소드
    		dataRecv(){
    			axios.get('http://localhost:8080/web/databoard/list_vue.do',{
    				params:{
    					page:this.curpage // this 필수 
    				}
    			}).then(response=>{
