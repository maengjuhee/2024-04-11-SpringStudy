...
public class BoardDAO {
    ...
    *  @Select("SELECT no,subject,name,TO_CHAR(regdate,'yyyy-mm-dd') as dbday,hit,num "
	   ...
	   public List<BoardVO> boardListDate(@Param("start") int start,@Param("end") int end);
	   
	   @Select("SELECT COUNT(*) FROM vue_board")
	   public int boardTotalPage();
    */
    public List<BoardVO> boardListDate(int start,int end)
    {
    	return mapper.boardListDate(start, end);
    }
    public int boardTotalPage()
    {
     ...
     * @Insert("INSERT INTO vue_board(no,name,subject,content,pwd) "
			   +"VALUES(vb_no.seq.nextval,#{name},#{subject},#{content},#{pwd})")
